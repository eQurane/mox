...
overview: Скрыть недоступную исполнителю карточку создания коллекции на экране проекта (и при желании — на экране ТЗ для единообразия), добавить карточки и маршрут загрузки медиа как у менеджера/админа, расширив `POST /api/media` для роли «Исполнитель» при активном членстве в проекте; затем обновить правила и отчётную документацию.
todos:
...
- Блок `if (isContractor) { ... }` не трогать — для исполнителя лишних ограничений по типу ТЗ не добавляем (в отличие от подрядчика).
