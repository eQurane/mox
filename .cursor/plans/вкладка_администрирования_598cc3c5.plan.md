...
- `GET /api/admin/overview` — агрегаты по БД (пользователи по статусам, проекты, задания, коллекции, медиа по статусам). **Без суммы размеров из БД.** При желании — опционально `storageLargeFilesCount` из того же сканера, что и раздел медиа (или не дублировать — достаточно отдельного эндпоинта ниже).

**Проблемы:**

- `GET /api/admin/issues` — проекты без активных участников; счётчик/список пользователей «На подтверждении»; по желанию — Задания без коллекций / коллекции без медиа (только `SELECT`, без DDL).
