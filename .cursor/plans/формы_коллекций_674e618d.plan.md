...
overview: Добавить REST для карточки коллекции (просмотр с медиа, создание, редактирование), hash-маршруты и страницы по образцу проекта/задания, диплинки создания с проекта и с экрана задания, картоčку «добавить медиа» с передачей collectionId в query, обновить ссылки на карточках и синхронизировать правила документации.
todos:
...
- **Загрузка файлов:** в репозитории нет `POST /api/media` / `multer`; медиа только читаются. Реализацию самой загрузки в этот объём не включать: на экране коллекции — та же **карточка «Добавить медиа»**, что на проекте/задания, со **ссылкой** `#/project/:projectId/media/new?collectionId=:id` (диплинк для будущей формы). При желании позже доработать [`projectFormStub`](c:\Users\eQurane\VSCode\mox\client\js\pages\projectFormStub.js) / форму медиа, читая `collectionId` из hash query.
...
2. **`POST /api/collections`**  
   ...
   - Проверка: задача существует; проект задачи «видим» редактору через ту же логику, что при записи в задание (см. `fetchProjectDatesIfVisible` / проверку в `tasks.js`).  
...
Ветка `project/:id` (числовой id), аналогично Задание:

| Путь | Действие |
|------|----------|
| `project/:id/collections/new` | `renderCollectionNewPage(appRoot, id)` — только Админ/Менеджер |
| `project/:id/tasks/:taskId/collections/new` | `renderCollectionNewPage(appRoot, id, taskId)` — только Админ/Менеджер; **диплинк с экрана задания** |
| `project/:id/collections/:collectionId` | `renderCollectionDetailPage` — роли как у просмотра проекта/задания (не редирект Клиента с проекта) |
...
1. **`collectionNew.js`**  
   - Без `taskId` в URL: загрузить `fetchProjectById`, `fetchTasks({ projectId })`; форма — выбор задания (`select`), название, описание; отправка `createCollection`; успех → `#/project/:projectId/collections/:newId` (или на проект — лучше на карточку коллекции).  
   - С `taskId`: загрузить `fetchTaskById`; проверить `task.projectId === projectId`; иначе сообщение + ссылка «в правильный проект» (как в taskDetail); поле задание не редактируется (подпись с именем задания).
...
- [`taskDetail.js`](c:\Users\eQurane\VSCode\mox\client\js\pages\taskDetail.js): `hrefCollectionsNew` заменить на `#/project/${projectId}/tasks/${taskId}/collections/new`; карточки коллекций сделать ссылкой на `#/project/${projectId}/collections/${c.id}` (класс как у карточки задания с `project-card--link`).  
