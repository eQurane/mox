...
2. **[`client/js/nav/dashboardTabs.js`](c:/Users/eQurane/VSCode/mox/client/js/nav/dashboardTabs.js)**  
   ...
   - **Задания** и **Коллекции** для клиента по‑прежнему скрыты; для подрядчика — без вкладки «Медиа» (как сейчас по смыслу ограничений).

3. **[`client/js/pages/projectDetail.js`](c:/Users/eQurane/VSCode/mox/client/js/pages/projectDetail.js)**  
   - Только для блока **«Мультимедиа»**: показать заголовок/кнопку перехода на **`#/media?projectId=…`** для **Клиента** (сейчас `hideGlobalBrowseLinks` скрывает и этот блок). задание и коллекции для клиента не трогать.
