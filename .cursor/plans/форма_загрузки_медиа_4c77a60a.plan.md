...
- На экране коллекции кнопка «Добавить медиа» уже только для админа/менеджера ([`collectionDetail.js`](client/js/pages/collectionDetail.js)); на проекте и задание кнопка показывается всем, кроме клиента — при текущей модели API исполнитель получит **403**.
...
1. **Новая страница** [`client/js/pages/mediaNew.js`](client/js/pages/mediaNew.js) (экспорт `renderMediaNewPage(container, projectId, searchParams)`):
   ...
   - **`fetchCollections({ projectId })`** — список коллекций **этого проекта** для `<select>` (подписи: имя коллекции + контекст задания из полей ответа API при необходимости).
