...
overview: Добавить защищённый маршрут `#/collections` со списком коллекций (как `#/tasks`), новый `GET /api/collections` с фильтрами `q`, `projectId`, `taskId` и правилами видимости как у задач, плюс входы с экрана проекта и со списка ТЗ.
todos:
  - id: api-collections
  ...
  - id: client-page-api
    ...
    status: completed
  - id: deeplinks-ui
    content: "projectDetail: секция Коллекции + ссылка с карточки ТЗ; tasksList: ссылка с карточки ТЗ"
    status: completed
  - id: tabs-jsdoc
...
- **[client/js/pages/collectionsList.js](c:\Users\eQurane\VSCode\mox\client\js\pages\collectionsList.js)** — экспорт `renderCollectionsListPage(container, searchParams)`:
  ...
  - После `fetchMe` / обновления сессии (как на ТЗ).
  - **Фильтры:** текстовый `q`; селект **Проект** (`fetchProjects`); селект **ТЗ** — опции из **`fetchTasks({ projectId })`** когда выбран проект (если проект «Все» — селект ТЗ пустой/заблокирован или одна опция «Все»). При смене проекта сбрасывать `taskId`, если выбранное ТЗ не входит в новый список.
  ...
  - **Карточка коллекции:** те же базовые классы, что у задачи (**`project-card project-card--static`**), плейсхолдер медиа; текст: название, описание, строки «Проект: …», «ТЗ: …», даты создания/изменения (как на [projectDetail.js](c:\Users\eQurane\VSCode\mox\client\js\pages\projectDetail.js) `buildCollectionCard`).
...
## 4. Диплинки с экрана проекта и со списка ТЗ
...
- Вручную: открыть `#/collections`; с проекта — `#/collections?projectId=…`; с карточки ТЗ — `#/collections?taskId=…`; поиск и смена фильтров обновляют hash; неавторизованный пользователь уходит на `#/login`.
