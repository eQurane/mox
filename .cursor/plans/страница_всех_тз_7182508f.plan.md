---
name: Страница всех ТЗ
overview: Экран `#/tasks` и `GET /api/tasks` с фильтрами; на главной — только переключатель «Проекты» (#/home) и «ТЗ» (#/tasks); со страницы проекта — `#/tasks?projectId=…` (заголовок секции и `list-24.svg`). Коллекции и медиа вне scope этой итерации (отдельные реестры и API позже).
todos:
  - id: api-get-tasks
  ...
  - id: client-api-tasks
    ...
    status: completed
  - id: tasks-page-ui
    content: tasksList.js — шапка с двумя табами как на главной (Проекты, ТЗ), фильтры, debounce, сетка, sync hash (?projectId=…)
    status: completed
  - id: router-nav
    content: "app.js: hash query, защищённый route tasks, renderTasksPage"
    status: completed
  - id: home-nav
    content: home.js — только табы Проекты (#/home) и ТЗ (#/tasks), активное состояние на дашборде
    status: completed
  - id: link-from-project
    content: "projectDetail.js: заголовок «Технические задания» + кнопка с /icons/list-24.svg ведут на #/tasks?projectId=…; карточка «Добавить ТЗ» по-прежнему на …/tasks/new"
    status: completed
  - id: css-filters
    content: main.css — панель фильтров ТЗ, ряд заголовка секции ТЗ на проекте, стили двух вкладок в шапке (.app-header__tab--current и др.)
    status: completed
  - id: sync-rules-docs
...
# Страница просмотра всех ТЗ (задачи)

## Контекст

В доменной модели **ТЗ соответствуют строкам таблицы `tasks`** ([`database-schema.mdc`](c:/Users/eQurane/VSCode/mox/.cursor/rules/database-schema.mdc)): связь с проектом `project_id`, статус задачи `status_id` → `statuses_tasks`, дата **`deadline`**. Отдельного ресурса «ТЗ» в API сейчас нет — данные задач уже отдаются внутри [`GET /api/projects/:id`](c:/Users/eQurane/VSCode/mox/server/src/routes/projects.js); для глобального списка с фильтрами нужен **новый эндпоинт**.

Карточный паттерн и разметка: как на главной ([`client/js/pages/home.js`](c:/Users/eQurane/VSCode/mox/client/js/pages/home.js) — `.projects-grid`, `.project-card`) и блок ТЗ на проекте ([`client/js/pages/projectDetail.js`](c:/Users/eQurane/VSCode/mox/client/js/pages/projectDetail.js) — `buildTaskCard`, `TASK_STATUS_SLUG`, `.project-card--static`).
...
sequenceDiagram
  ...
  Note over Browser,TasksPage: Клик «Технические задания» или кнопка list-24
...
| **ТЗ**      | `#/tasks` — список всех технических заданий. |
...
Дополнительные входы: **главная** (см. выше); **со страницы проекта** для ТЗ — как в следующем разделе.

## Клиент: страница списка ТЗ

**[`client/js/pages/tasksList.js`](c:/Users/eQurane/VSCode/mox/client/js/pages/tasksList.js)**

- Каркас: `main.dashboard`, шапка с **двумя** вкладками (**Проекты · ТЗ**) как на главной, активна **ТЗ**; заголовок «Технические задания»; опционально кнопка «Назад» на `#/home`; при необходимости ссылка «К проектам».
- Загрузка:
...
## Точка входа со страницы проекта (ТЗ)

В [`client/js/pages/projectDetail.js`](c:/Users/eQurane/VSCode/mox/client/js/pages/projectDetail.js) секция ТЗ сейчас использует [`sectionHeading(hrefTasks, 'Технические задания')`](c:/Users/eQurane/VSCode/mox/client/js/pages/projectDetail.js), где `hrefTasks` указывает на `#/project/:id/tasks/new`.

**Изменить поведение:**

1. Заголовок **«Технические задания»** (кликабельная надпись) вести на **`#/tasks?projectId=<текущий projectId>`** — экран просмотра всех ТЗ с предвыбранным проектом.
2. Справа от заголовка в одном ряду (`project-detail__section-title` или обёртка-`flex`) добавить **кнопку** (например `button button-ghost button-icon`, `aria-label` вроде «Список всех технических заданий проекта») с иконкой [`client/icons/list-24.svg`](c:/Users/eQurane/VSCode/mox/client/icons/list-24.svg) — тот же **`#/tasks?projectId=…`**.
3. Ссылку на создание нового ТЗ **не удалять**: карточка **«Добавить ТЗ»** при пустой сетке и существующий маршрут `#/project/:id/tasks/new` остаются; при необходимости добавить вторичную точку перехода к `#/project/…/tasks/new` (например небольшая текстовая ссылка «Добавить ТЗ», если секция уже не ведёт на new через заголовок).

Итого: два элемента (надпись + кнопка-список) ведут на список ТЗ с отфильтрованным проектом.

## Стили

В [`client/styles/main.css`](c:/Users/eQurane/VSCode/mox/client/styles/main.css) — блок раскладки фильтров (flex/grid), отступы; раскладка **ряда заголовка секции ТЗ** (flex, выравнивание `h2` + кнопка с иконкой, не ломая остальные секции).
...
| Изменить | `client/js/pages/projectDetail.js` (ТЗ-секция) |
