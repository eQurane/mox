---
name: Форма создания задания
overview: "Реализовать полноценную форму `#/project/:id/tasks/new` по образцу формы проекта: новый экран на клиенте, плюс на бэкенде `GET /api/tasks/create-options` и `POST /api/tasks` с валидацией дедлайна в границах проекта и роли исполнителя задания."
todos:
  - id: backend-post-get-options
    content: Добавить GET /api/tasks/create-options и POST /api/tasks — только Админ/Менеджер; валидация проекта, дедлайн в [start,end], roleId ∈ {Исполнитель, Внешний подрядчик} (тип исполнителя задания)
    status: completed
  - id: client-api-task-new
    content: "Расширить client/js/api/tasks.js: fetchTaskCreateOptions, createTask"
    status: completed
  - id: client-page-task-new
    content: Создать client/js/pages/taskNew.js; скрыть «Добавить задание» у не-админ/не-менеджер в projectDetail.js
    status: completed
  - id: client-router
...
- Сейчас «Добавить задание» в [`projectDetail.js`](client/js/pages/projectDetail.js) видно всем, кроме **Клиент** — после изменения правок: показывать **только Админ и Менеджер** (как кнопка редактирования проекта). Список задание (`GET /api/tasks`) для **Клиент** и **Внешний подрядчик** по-прежнему запрещён; **Исполнитель** может смотреть задания, но **не создаёт** их.

## Права доступа

| Роль | POST создание задания, GET create-options |
|------|----------------------------------------|
...
1. **`GET /api/tasks/create-options`** (`requireAuth`):
   ...
   - **200:** `{ statuses: [{ id, name }], taskRoles: [{ id, name }] }` — статусы из `statuses_tasks`; **taskRoles** — это допустимые **типы исполнения задания** в БД: роли **«Исполнитель»** и **«Внешний подрядчик»** из `roles` (`WHERE name IN (...)`), порядок фиксированный.
...
- Разметка как у [`projectNew.js`](client/js/pages/projectNew.js): `main.page.register-page.project-new-page`, `register-card`, заголовок «Новое задание», кнопка «Назад» → `#/project/:id`.
...
- Поля:
  ...
  - Роль задания — `select` из `taskRoles` (два варианта);
...
- Текстовая ссылка **«Добавить задание»** и карточка **«Добавить задание»** в пустой сетке — **только Админ и Менеджер**. Заголовок секции и переход на **`#/tasks?projectId=`** (список заданий) оставить для **Исполнителя** как сейчас для не-клиентов (у **Клиента** по-прежнему без ссылок на задания).
